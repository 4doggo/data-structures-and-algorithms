...
const firstLetters = (arr) => {
  const letters = arr.reduce((accum, value) => {
    accum.push(value.charAt(0));
    return accum;
  }, []);
  return letters;
};
...
const findHappiness = (arr) => {
  const happy = arr.reduce((accum, value) => {
    if (value.includes(':)')) {
      accum.push(value);
    }
    return accum;
...
const standardizePhoneNumbers = (arr) => {
  const newArr = arr.map(num => {
    let newNum = (num.substring(1, 4) + num.substring(6, 9) + num.substring(10, 14));
    return newNum;
  });
  return newArr;
};
...
const onlyOddChars = (str) => {
  const odds = str.split('').filter((value, index) => index % 2 === 1).join('');
  return odds;
};
...
const allHappy = (arr) => {
  const happy = (value) => value.includes(':)');
...
const findAnything = (arr, target) => {
  const foundThings = arr.reduce((accum, value) => {
...
const findEvery = (arr, target) => {
  const allOrNone = (value) => value.includes(target);
  return arr.every(allOrNone);
};
